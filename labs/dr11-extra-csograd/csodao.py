...
def getAll(dataset):   ############3
    url = urlBegining + dataset + urlEnd
    response = requests.get(url)
    return response.json()

def getFormattedAsFile(dataset): ###############################start here (1)
    with open("cso-formatted.json", "wt") as fp:
        print(json.dumps(getFormatted(dataset)), file=fp)
  

def getFormatted(dataset): #############2 copy from url and paste into rawdump.json to view 
    data = getAll(dataset)
    ids = data["id"]
    values = data["value"]
    ...
    for dim0 in range(0, sizes[0]):
        ...
        print(label0)
        for dim1 in range(0, sizes[1]): # should have 5
            currentId = ids[1]
            ...
            for dim2 in range(0, sizes[2]):
                ...
                for dim3 in range(0, sizes[3]):
                    ...
                    for dim4 in range(0, sizes[4]):
                        ...
                        db_values = (label1, label2, label3, label4, int(values[valuecount]) ) # label1 = num of grad, 2 = 2010, 2011, 2012..., 3 = all fields of education, 4= 
